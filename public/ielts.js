...
async function loadAllIELTSData() {
    try {
        for (const stage of IELTS_STATE.stages) {
            // Load trainings
            const trainingsResponse = await fetch(`${window.API_BASE_URL}/ielts?type=training&stage=${stage}`);
            const trainings = await trainingsResponse.json();
            IELTS_STATE.trainings[stage] = trainings || [];
            
            // Load audio tracks
            const tracksResponse = await fetch(`${window.API_BASE_URL}/ielts?type=audio&stage=${stage}`);
...
async function uploadAudioForTraining(stage) {
    ...
    try {
        const response = await fetch(`${window.API_BASE_URL}/ielts?type=audio`, {
            method: 'POST',
            body: formData
...
async function saveTraining(stage) {
    ...
    try {
            const response = await fetch(`${window.API_BASE_URL}/ielts?type=training`, {
            method: 'POST',
